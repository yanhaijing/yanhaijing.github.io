...
由于JSON.stringify的方法走不通，另一种思路是自己写代码判断结构相等，其原理是依次遍历递归比较两个树是否相等，也可以使用社区中别人写好的库，比如[isequal](https://github.com/jsmini/isequal)。

鉴于篇幅，本文不在给出递归判断结构相等的代码，在下一篇文章中，给大家带来isequal的源码分析。
